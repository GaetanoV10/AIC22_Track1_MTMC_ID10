...
def run(sequence_dir, detection_file, output_file, track_thresh, match_thresh,
        nms_max_overlap, min_detection_height, max_cosine_distance,
        display, max_frame_idx, mcmt_cfg, use_embedding=True):
    ...
    """
    print(f"Is use embedding distance? {use_embedding}")
    ...
    tracker = BYTETracker(track_thresh, match_thresh, frame_rate=seq_info["frame_rate"])
    results = []
    ...
    def frame_callback(vis, frame_idx):
        ...
        # Update tracker.
        online_targets = tracker.update(detections, feats, cid, use_embedding=use_embedding)
...
def save_pickle(output_file, results, sequence_name, mcmt_cfg):
    ...
    out_dir = os.path.join(cfg.DATA_DIR, 'mot_result')
    if not os.path.exists(f'{out_dir}/{sequence_name}'):
        os.makedirs(f'{out_dir}/{sequence_name}')
    feat_pkl_file = f'{out_dir}/{sequence_name}/{sequence_name}_mot_feat_raw.pkl'
    mot_feat_dic = {}
    for row in results:
...
def parse_args():
    ...
    parser.add_argument("--use_embedding", action="store_true", help="using embedding distance")
...
if __name__ == "__main__":
    ...
    feat_dir = os.path.join(cfg.DATA_DIR, 'feature_merge')
    mot_dir = os.path.join(cfg.DATA_DIR, 'mot_result')
    if not os.path.exists(mot_dir):
        os.makedirs(mot_dir)
    args.detection_file = os.path.join(feat_dir, args.seq_name,
                                       f'{args.seq_name}_dets_feat.pkl')
    args.output_file = os.path.join(mot_dir, args.seq_name,
                                    f'{args.seq_name}_mot.txt')

    run(
        args.sequence_dir, args.detection_file, args.output_file,
        args.track_thresh, args.match_thresh, args.nms_max_overlap, args.min_detection_height,
